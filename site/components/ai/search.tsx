...
import { AIPanelBackdrop } from '@/components/shader-art';
    ...
    <form {...props} className={cn('flex items-start pe-2', props.className)} onSubmit={onStart}>
      ...
      {isLoading ? (
        <button
          ...
          onClick={stop}
        >
          <RiLoader4Line className="size-4 animate-spin text-fd-muted-foreground" />
          Abort Answer
        </button>
      ) : (
        <button
  ...
  return (
    <>
      ...
      {actualOpen && (
        <div
          className={cn(
            'relative overflow-hidden z-30 bg-fd-card text-fd-card-foreground [--ai-chat-width:400px] 2xl:[--ai-chat-width:460px]',
