...
Sarvam's chat endpoint speaks the OpenAI wire, so it's a compatible
factory:

```swift
let sarvam = OpenAICompatibleProvider.sarvam()
...
see [OpenAI-compatible](/docs/providers/openai-compatible).
...
language; set it on the model or per request:
...
models. Language and mode ride through `providerOptions`:
