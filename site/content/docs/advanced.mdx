...
`MCPClient` speaks Streamable HTTP (JSON-RPC over POST, SSE responses,
session ids) and bridges server tools straight into the loop:
...
Bridged tools validate arguments against the server's schemas and return
results into the same loop as local tools.
